...
# Define transitions with outputs
transitions = [
    ...
    {'trigger': 'unplug_vehicle', 'source': 'Finished', 'dest': 'Idle', 'conditions': 'vehicle_unplugged'},
]
...
MealyMachine = MachineFactory.get_predefined(nested=True)

# Define the EV Charging Station class
class EVChargingStation(object):
    def __init__(self):
        self.machine = MealyMachine(model=self, states=states, transitions=transitions, initial='Idle', outputs=outputs)
...
def main():
    ...
    while True:
        ...
        if selected_transition in charging_station.machine.get_triggers():
            ...
            print(f"Output of the transition: {charging_station.machine.get_state()}")
            print(f"Available transitions from '{charging_station.state}': {charging_station.machine.get_triggers()}")
        else:
